...
/**
 * Handles collisions between all types of objects. Implements the rules of
 * spacewar for bouncing off objects versus picking them up, etc. Implemented in
 * a central place to make it easy to change as need be.
 * 
 * Note: people have observed ships "sticking" to asteroids or bases when they
 * hit them at low velocity. This isn't a bug in the physics simulator. If you
 * have a lower mass object hitting an object of much higher mass at a low
 * velocity, you would expect the low mass one to essentially stick to the
 * higher mass one. The solution is to move quickly away from the objects after
 * you collide.
 * 
 * 
 * @author amy
...
public class CollisionHandler {
	...
	public void collide(AbstractObject object1, AbstractObject object2, Toroidal2DPhysics space) {
		...
		// if the object is a flag, handle either moving it (same team) or picking it up
		// (opposite team)
		...
		// handle mineable asteroid collisions (e.g. mine them if needed and don't
		// elastically collide, e.g
		...
		// Handle AiCore Collisions with ships (Destroy them if same team, collect them
		// if different team) and don't elastically collide
		...
		// Handle AiCore collisions with Beacons (Restoring the energy of the AiCore)
		// and don't elastically collide
		...
		// Handle AiCore collisions with bases (core is collected) and then collides if
		// it is an enemy
	...
	/**
	 * Collide with a Flag. The rules are as follows: 1) If a ship touches a flag
	 * from their own team, it disappears and reappears at the next time step in a
	 * random alcove 2) If a ship touches a flag from a team other than their own,
	 * that ship picks up the flag.
	 * 
	...
	/**
	 * Collide a ship and a drone. If it is a friendly ship, the friendly ship will
	 * beam over their resources, cores, and held flags.
	 * 
	 * @param drone
	 * @param ship
	 */
	private void droneCollision(Drone drone, Ship ship) { // herr0861 edit
		if (ship.getTeamName().equalsIgnoreCase(drone.getTeamName())) {
			...
			if (ship.isCarryingFlag()) {
				...
				drone.getFlag().pickupFlag(drone); // the pickup method in the flag will automatically cause it to not
													// be carried by the ship when the drone has it
			}
	...
	private void missileCollision(Missile missile, AbstractObject object2) {
		...
		if (object2.getClass() == Ship.class) {
			...
			if (!ship.isShielded()) {
				...
				if (ship.getEnergy() <= 0) {
					// if you killed the ship, only count the final amount of damage needed to kill
					// it
			...
			if (ship.getEnergy() <= 0) {
				// System.out.println("ship " + firingShip.getTeamName() + " stealing
				// resourcesAvailable " + shipMoney + " from " + ship.getTeamName() +
				// ship.getId());
		...
		if (object2.getClass() == Base.class) {
			...
			if (!base.isShielded()) {
				...
				if (base.getEnergy() <= 0) {
					...
					// System.out.println("Firing at a dead base - should give only " + (int)
					// -initialEnergy + " in damage");
				} else {
	...
	/**
	 * If a base and core collide, the core destroyed if it is a friendly core and
	 * elastically collides if it is an enemy. Collision is handled higher up in the
	 * function
	 * 
	 * @param core
	 * @param base
	...
	public void damageAiCore(AiCore core) {
		// double penalty = -Math.abs(COLLISION_PENALTY *
		// core.getPosition().getTotalTranslationalVelocity());
	...
	public void mineAsteroid(Asteroid asteroid, Ship ship) {
		...
		// System.out.println("ship " + ship.getTeamName() + ship.getId() +" now has
	...
	public void droneCollision(Drone drone) {
		double penalty = -Math.abs((COLLISION_PENALTY / 3) * drone.getPosition().getTotalTranslationalVelocity()); // The
																													// drone
																													// is
																													// made
																													// of
		...
		// Currently no need to track how much damage the drone took for now, but we
		// should implement it into damage done for aggressive clients.
	...
	public void baseCollision(Base base, AbstractObject object) {
		if (object.getClass() == Ship.class) {
			...
			if (ship.getTeamName().equalsIgnoreCase(base.getTeamName())) {
				...
				// System.out.println("ship " + ship.getTeamName() + ship.getId() + " left
				// resourcesAvailable at base and now has resourcesAvailable " +
				// ship.getMoney());
			}
	...
	private void elasticCollision2D(AbstractObject object1, AbstractObject object2, Toroidal2DPhysics space) {
		...
		// now get the vector from the first to the second, get the unit normal and
		// tangent
	 ...
	 * @param space   handle to space for distance calculations
	 */
	private void elasticCollision2DWithNonMoveableObject(AbstractObject movingObject, AbstractObject stationaryObject,
		...
		// now get the vector from the first to the second, get the unit normal and
		// tangent
		Vector2D distanceVec = space.findShortestDistanceVector(movingObject.getPosition(),
				stationaryObject.getPosition());
		Vector2D unitNormal = distanceVec.getUnitVector();
		Vector2D unitTangent = new Vector2D(-unitNormal.getYValue(), unitNormal.getXValue());
	 ...
	 * 
	 * let u1 = velocity of item 1 before collision, m1 = mass of item 1 u2 =
	 * velocity of item 2 before collision, m2 = mass of item 2 v1 = velocity of
	 * item 1 after collision v2 = velocity of item 2 after collision
	 * 
	 * v1 = ((u1 * (m1 - m2)) + (2 * m2 * u2)) / (m1 + m2) v2 = ((u2 * (m2 - m1)) +
	 * (2 * m1 * u1)) / (m1 + m2)
	 * 
	 */
	...
	/**
	 * Adjusts the centers of object1 and object2 so that their collision occurs
	 * when they are touching, not overlapping
	 * 
	...
	private void adjustCentersAtCollision(AbstractObject object1, AbstractObject object2, Toroidal2DPhysics space) {
		...
		// ==>
		// Solve[r1 + r2 == Sqrt[((x2 + u2*t)-(x1 + u1*t))^2 + ((y2 + v2*t)-(y1 +
		// v1*t))^2], t]
		// ==>
		// Gives two results for t

		double tPlus = (-2 * u1 * x1 + 2 * u2 * x1 + 2 * u1 * x2 - 2 * u2 * x2 - 2 * v1 * y1 + 2 * v2 * y1 + 2 * v1 * y2
				- 2 * v2 * y2 + Math.sqrt( // +
						Math.pow(2 * u1 * x1 - 2 * u2 * x1 - 2 * u1 * x2 + 2 * u2 * x2 + 2 * v1 * y1 - 2 * v2 * y1
								- 2 * v1 * y2 + 2 * v2 * y2, 2)
								- 4 * (Math.pow(u1, 2) - 2 * u1 * u2 + Math.pow(u2, 2) + Math.pow(v1, 2) - 2 * v1 * v2
										+ Math.pow(v2, 2))
										* (Math.pow(x1, 2) + Math.pow(x2, 2) + Math.pow(y1, 2) + Math.pow(y2, 2)
												- Math.pow(r1, 2) - Math.pow(r2, 2) - 2 * r1 * r2 - 2 * x1 * x2
												- 2 * y1 * y2)))
				/ (2 * (Math.pow(u1, 2) - 2 * u1 * u2 + Math.pow(u2, 2) + Math.pow(v1, 2) - 2 * v1 * v2
						+ Math.pow(v2, 2)));

		double tMinus = (-2 * u1 * x1 + 2 * u2 * x1 + 2 * u1 * x2 - 2 * u2 * x2 - 2 * v1 * y1 + 2 * v2 * y1
				+ 2 * v1 * y2 - 2 * v2 * y2 - Math.sqrt( // -
						Math.pow(2 * u1 * x1 - 2 * u2 * x1 - 2 * u1 * x2 + 2 * u2 * x2 + 2 * v1 * y1 - 2 * v2 * y1
								- 2 * v1 * y2 + 2 * v2 * y2, 2)
								- 4 * (Math.pow(u1, 2) - 2 * u1 * u2 + Math.pow(u2, 2) + Math.pow(v1, 2) - 2 * v1 * v2
										+ Math.pow(v2, 2))
		...
		// Amy McGovern: added 2.0 * duration instead of just duration because of double
		// precision issues
		double time = 0;
		// System.out.println("Time adjustment solutions tplus = " + tPlus + " tminus =
		// " + tMinus);
