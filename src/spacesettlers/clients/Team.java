...
/**
 * A team holds the ships and a pointer to the client that controls the ships.
 * They are separated for security inside the client (to keep the client from
 * directly manipulating ships)
 * 
 * @author amy
 */
public class Team {
	...
	/**
	 * herr0861 edit The set of drones owned by this team
	 */
	Set<UUID> teamDroneIDs;

	/**
	 * The set of bases associated with this team (bases are not stored directly
	 * because they point to team and then cloning causes a stack overflow)
	 */
	Set<UUID> teamBaseIDs;

	/**
	 * A set of all the ids associated with the team (used to verify power ups)
	 */
	Set<UUID> teamIDs;
	...
	/**
	 * The current costs for this team to buy items (some change as you buy more and
	 * more of them)
	 */
	...
	/**
	 * herr0861 edit Add a drone to the team
	 * 
	 * @param drone
	 */
	...
	/**
	 * herr0861 edit Return the list of drones.
	 * 
	 * @return
	 */
	...
	/**
	 * herr0861 edit Remove the drone because it died
	 * 
	 * @param drone
	 */
	...
	public Map<UUID, AbstractAction> getTeamMovementStart(Toroidal2DPhysics space) {
		...
		// HashMap<UUID, AbstractAction> teamActions = teamClient.getAction(clonedSpace,
		// clonedTeamShips);
		// return teamActions;
	}

	/**
	 * Allows the client to do cleanup after an action and before the next one (if
	 * needed)
	 * 
	...
	public Map<UUID, PurchaseTypes> getTeamPurchases(Toroidal2DPhysics space) {
		...
		Future<Map<UUID, PurchaseTypes>> future = executor.submit(new Callable<Map<UUID, PurchaseTypes>>() {
			public Map<UUID, PurchaseTypes> call() throws Exception {
				return teamClient.getTeamPurchases(clonedSpace, clonedActionableObjects, clonedResources,
						clonedPurchaseCost);
			}
		});
	...
	public Map<UUID, SpaceSettlersPowerupEnum> getTeamPowerups(Toroidal2DPhysics space) {
		...
		// System.out.println("exec " + executor.isTerminated());
		Future<Map<UUID, SpaceSettlersPowerupEnum>> future = executor
				.submit(new Callable<Map<UUID, SpaceSettlersPowerupEnum>>() {
	...
	public Set<SpacewarGraphics> getGraphics() {
		...
		Future<Set<SpacewarGraphics>> future = executor.submit(new Callable<Set<SpacewarGraphics>>() {
	 ...
	 * the client to return a proper mouse listener. It has to be pushed up here to
	 * avoid the exploits.
	 *
	 * @param mouseTransform takes an affine transformation used to scale the
	 *                       graphics: must be used to scale mouse clicks
	 * 
	...
	/**
	 * Increment the team's currently available resources (this is decremented
	 * through buying elsewhere)
	 * 
	...
	/**
	 * Decrement the team's currently available resources (for purchasing or
	 * stealing)
	 * 
	...
	/**
	 * Return the total of the resources collected so far (used for scoring, can be
	 * adjusted to be a weighted sum later)
	 * 
