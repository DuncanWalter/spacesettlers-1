...
/**
 * An example chromosome for a space settlers agent using genetic algorithms /
...
public class ExampleGAChromosome {
	 ...
	 * @return
	 */
	public AbstractAction getCurrentAction(Toroidal2DPhysics space, Ship myShip, ExampleGAState currentState,
			Random rand) {
		if (!policy.containsKey(currentState)) {
			...
			if (rand.nextBoolean()) {
				...
				// System.out.println("Moving to nearestMineable Asteroid " +
				// myShip.getPosition() + " nearest " +
				// currentState.getNearestMineableAsteroid().getPosition());
				policy.put(currentState,
						new MoveToObjectAction(space, myShip.getPosition(), currentState.getNearestMineableAsteroid()));
			}
		}
