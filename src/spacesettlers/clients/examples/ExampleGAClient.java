...
/**
 * Demonstrates one idea on implementing Genetic Algorithms/Evolutionary
 * Computation within the space settlers framework.
 * 
 * @author amy
 *
 */
...
public class ExampleGAClient extends TeamClient {
	...
	public Map<UUID, AbstractAction> getMovementStart(Toroidal2DPhysics space,
		...
		// loop through each ship and send it the current policy from the chromosome. If
		// the chromosome
		// hasn't seen a state before, it will pick an abstract action (you should make
	...
	public void getMovementEnd(Toroidal2DPhysics space, Set<AbstractActionableObject> actionableObjects) {
		...
		// if the step counter is modulo evaluationSteps, then evaluate this member and
		// move to the next one
		if (steps % evaluationSteps == 0) {
			// note that this method currently scores every policy as zero as this is part
			// of
	...
	/**
	 * Initialize the population by either reading it from the file or making a new
	 * one from scratch
	 * 
	 * @param space
	 */
	...
	public void initialize(Toroidal2DPhysics space) {
		...
		// try to load the population from the existing saved file. If that failes,
		// start from scratch
		try {
