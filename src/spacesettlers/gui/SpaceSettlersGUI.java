...
public class SpaceSettlersGUI {
	...
	public SpaceSettlersGUI(SpaceSettlersConfig config, SpaceSettlersSimulator simulator) {
		...
		/*
		 * mainFrame.setLayout(new GridLayout(1,2)); mainFrame.add(mainComponent);
		...
		// System.out.println("Playing field size height " + config.getHeight() + "
		...
		if (screenWidth < requestedWidth || screenHeight < requestedHeight) {
			...
			mainComponent.setPreferredSize(
					new Dimension((int) (scale * config.getWidth()), (int) (scale * config.getHeight())));

			// System.out.println("Original preferred size for info is " +
			// infoPanel.getPreferredSize());
			infoScrollPane
					.setPreferredSize(new Dimension((int) infoSize.getWidth(), (int) (scale * config.getHeight())));
			// System.out.println("Setting preferred size for info to " +
			// infoPanel.getPreferredSize());
		...
		// mainFrame.setPreferredSize(new Dimension(Math.min((int)screenWidth,
		// requestedWidth),
		...
		// System.out.println("Info component width x height " + infoPanel.getWidth() +
		// " x " + infoPanel.getHeight());
		// System.out.println("Main component width x height " + mainPanel.getWidth() +
		// " x " + mainPanel.getHeight());
	}

	/**
	 * Main GUI mouse interface. Pops up a info area in the info panel if the user
	 * LEFT clicks on an object
	 * 
	 * @author amy
	 *
	 */
	class GUIMouseListener extends MouseAdapter {
		...
		public void mouseReleased(MouseEvent e) {
			...
			// System.out.println("Transformed x, y " + newPoint.getX() + " , " +
			// newPoint.getY());

			// only listens to left clicks
			if (e.getButton() == MouseEvent.BUTTON1) {
				// get the set of all objects and figure out if the user clicked inside an
				// object
				...
				for (AbstractObject obj : allObjects) {
					// System.out.println("Object " + obj + " distance to click " +
					// space.findShortestDistance(clickPosition, obj.getPosition()) + " radius is "
					// + obj.getRadius());
