 ...
 * 
 * Note this action IGNORES the orientation inside the targetLocation! If you
 * want to move one way with your ship but point in a different way, you should
 * use MoveActionSeparateOrientation.
 * 
 * The action uses PD control to move the ship. It does this by:
 * 
 * 1) Orienting the ship to the target location 2) Accelerating to the target
 * location
 * 
 * @author amy
 */
public class MoveAction extends AbstractAction {
	/**
	 * If the acceleration is less than this number, the target is assumed to be
	 * reached
	 */
	 ...
	 * @param targetVelocity  the velocity the ship should be at when it reaches its
	 *                        target location
	 * @throws SpaceSettlersActionException
	 */
	public MoveAction(Toroidal2DPhysics space, Position currentLocation, Position targetLocation,
		...
		if (Double.isFinite(targetLocation.getX()) && Double.isFinite(targetLocation.getY())) {
			...
			System.out.println(
					"Error: you called MoveAction with a target location that is not finite. Using current instead.");
		...
		if (Double.isFinite(targetVelocity.getXValue()) && Double.isFinite(targetVelocity.getYValue())) {
			...
			System.out.println(
					"Error: you called MoveAction with a target velocity that is not finite. Using zero instead.");
	...
	/**
	 * Make a new move action and save the goal locations. Note the target velocity
	...
	/**
	 * This constructor assumes you will call the pd control functions directly so
	 * it does not initialize anything This is used by the unit tests but it could
	...
	/**
	 * Be careful setting these. They can break control. To be critically damped,
	 * the parameters must satisfy: 2 * sqrt(Kp) = Kv
	 * 
	...
	/**
	 * Proportional derivative controller. The equation is: acceleration = k_p *
	 * position error + k_v * velocity error
	 * 
	 * To be critically damped, the parameters must satisfy: 2 * sqrt(Kp) = Kv
	 *
	...
	/**
	 * Returns the error in orientation space (separate function so we can use it to
	 * decide when it has reached the goal)
	 * 
	 * @param space
	 * @param goalLoc
	 * @param currentLoc
	 ...
	 * @param goalAngularVelocity velocity you want to be at when you reach the goal
	 *                            location
	 * @param currentVelocity     your current velocity
	 * @return angular acceleration needed to move to the goal
	 */
	public double pdControlOrientToGoal(Toroidal2DPhysics space, Position goalLoc, Position currentLoc,
	...
	/**
	 * Accelerate to the goal location using pd control. Assumes you are already
	 * pointed at the goal. Returns the linear/translational acceleration needed to
	 * move to the goal.
	 * 
	 * @param goalLoc      location that you are trying to get to
	 * @param currentLoc   your current location
	 * @param goalVelocity velocity you want to be at when you reach the goal
	 *                     location
	 * @return the accleration vector needed to move to the goal
	 */
	public Vector2D pdControlMoveToGoal(Toroidal2DPhysics space, Position goalLoc, Position currentLoc,
		...
		// currentLoc.getTranslationalVelocity());
		...
		// System.out.println("Velocity error is " + velocityErrorX + " ," +
		// velocityErrorY);
	...
	/**
	 * Herr0861 edit. Overloading getMovement() to allow for handling of multiple
	 * object types. In this case, Drone. Move the drone to the goal by turning and
	 * moving to the goal at the same time.
	 * 
	 */
